...
GETPAT ; Get a patient
 S RTN="GETPAT^SDMAPI3(.RETURN,PAT,)" D EXSTPAT^ZZRGUSD2(RTN)
 ...
 Q
CHKCO ;
 S RTN="CHKCO^SDMAPI4(.RETURN,PAT,SD)" D EXSTPAT^ZZRGUSD2(RTN)
...
LSTDAYAP ;
 ...
 ;
 S RTN="GETPAPT^SDMAPI4(.RETURN,PAT,SD)" D EXSTPAT^ZZRGUSD2(RTN)
 ...
 ;
 S RTN="GETCAPT^SDMAPI4(.RETURN,PAT,SD)" D EXSTPAT^ZZRGUSD2(RTN)
 ...
 Q
GETPAT2 ;
 S RTN="GETPAT^SDMAPI4(.RETURN,PAT)" D EXSTPAT^ZZRGUSD2(RTN)
...
ANCTEST ;
 ...
 S RTN="ADDTSTS^SDMAPI4(.RETURN,PAT,SD,.LAB,.XRAY,.EKG)" D EXSTPAT^ZZRGUSD2(RTN)
 ...
 D CHKEQ^XTMUNIT(RETURN,1,"Unexpected error:")
 D CHKEQ^XTMUNIT($P(^DPT(+DFN,"S",+SD,0),U,3,5),+LAB_U_+XRAY_U_+EKG,"Invalid length")
 S RTN="DELTSTS^SDMAPI4(.RETURN,PAT,SD,,,)" D EXSTPAT^ZZRGUSD2(RTN)
...
REQESTS ;
 ...
 S RTN="GETAPCNS^SDCAPI1(.RETURN,PAT,105)" D EXSTPAT^ZZRGUSD2(RTN)
 S %=$$GETAPCNS^SDCAPI1(.RETURN,DFN,105)
 D CHKEQ^XTMUNIT(RETURN,1,"Unexpected error: "_$G(RETURN(0)))
 ;autorebook
 S RTN="AUTOREB^SDCAPI1(.RETURN,CLN,SD,CONS,1)" D EXSTCLN^ZZRGUSD2(RTN)
...
CANAPP ;
 ...
 D CHKEQ^XTMUNIT(RETURN,1,"Unexpected error: "_$G(RETURN(0)))
 ;
 S SD1=$P(DT,".")_".23",SD1=SD1_U_$$FMTE^XLFDT(SD1)
